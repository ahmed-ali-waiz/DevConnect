    ...
    >
      ...
      <nav className="flex-1 flex flex-col space-y-2">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `nav-item relative flex items-center space-x-4 px-4 py-3 rounded-xl transition-all duration-300 group
              ${isActive 
      ...
      {user ? (
        <div className="mt-auto pt-4 border-t border-(--border-glass)">
          <button className="w-full flex items-center justify-between p-2 rounded-xl hover:bg-(--bg-glass) transition-colors group">
            <div className="flex items-center space-x-3 overflow-hidden">
              <Avatar src={user.profilePic} alt={user.name} size="md" />
              <div className="hidden xl:flex flex-col items-start truncate text-left">
            ...
            <div onClick={handleLogout} className="hidden xl:block opacity-0 group-hover:opacity-100 transition-opacity text-(--text-muted) hover:text-red-400 p-2">
              <LogOutIcon size={18} />
            </div>
          </button>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-(--border-glass)">
          <NavLink to="/login">
            <Button variant="ghost" className="w-full justify-start xl:justify-center">
              Sign In
            </Button>
          </NavLink>
        </div>
